...
def size_meters(payload):
    ...
    if len(valid) > 1:
        warnings.append(
            "Only one meter type can be selected. "
    ...
    # any meter's own part number. Only one meter type is sized, so there is at
    # most one.
    eagle = None
    if eagle_requests:
        eagle = eagle_selection(eagle_requests[0], inlet_psi)
